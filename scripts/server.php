    ...
    if (!$client || ($client['amount'] != $dataPost->amount)) {
        ...
        if (!$email->sendEmail($data, true, $dataPost->order_id)) {
            ...
            if (!$email->sendEmail($data, true, $dataPost->order_id)) {
                sleep(10);
                if (!$email->sendEmail($data, true, $dataPost->order_id)) {
                    $sms->sendSMS('BurgerJoint', '380671345834', 'Помилка вiдправки листа. Телефон замовника '.$data->phoneNumber.'. Звернiться до администраторiв.');
                }
            }
        }
        $sms->sendSMS('BurgerJoint', $data->phoneNumber, 'Дякуємо. Ваше замовлення прийнято. З будь якими питаннями звертайтесь за телефоном +38 (067) 134 58 34');
